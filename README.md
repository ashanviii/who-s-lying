...
1. **Extract** — the profile text (or fetched URL content) is sent to GPT, which pulls out every discrete, checkable claim: titles, employers, dates, degrees, metrics, awards, leadership claims, etc.
2. **Research** — each claim is handed to a second GPT call (via the Responses API) with OpenAI's hosted **web search tool** turned on, so the model actively searches the public web for corroborating or contradicting evidence rather than guessing from training data.
...
No separate search API is required — claim verification uses OpenAI's built-in `web_search` tool, so the only credential you need is an OpenAI API key.

## Tech stack

- **Next.js 16** (App Router, TypeScript, Tailwind CSS v4) — one app for both the frontend and the API routes.
- **`openai`** — claim extraction and research/scoring both go through the Responses API (`client.responses.create`) using structured outputs (`text.format: json_schema`) plus the hosted `web_search` tool for the research call.
...
If `OPENAI_API_KEY` is not set, the app automatically runs in **demo mode**: every "Analyze a profile" click (and the "Try the demo profile" button) returns a pre-written, clearly-labeled example result (a fictional profile, fictional evidence, `example.com` source links) so you can see the full product experience — loading states, score gauge, claim cards, sharing — without any setup or API spend.

### Environment variables

| Variable | Required | Default | Description |
| --- | --- | --- | --- |
| `OPENAI_API_KEY` | For real analyses | — | Your OpenAI API key. Without it, the app runs in demo mode. |
| `OPENAI_MODEL` | No | `gpt-5` | Model used for both extraction and research/scoring calls. Swap in `gpt-4.1` or `gpt-4o-mini` for a cheaper/faster pipeline. |
...
src/
  ...
  lib/
    ...
    llm-client.ts               OpenAI client + structured-output helper (Responses API)
